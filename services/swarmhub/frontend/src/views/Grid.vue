  ...
  <div class="grid">
    <section class="section">
      <div class="tile is-ancestor">
        <GridList :selectedGridID="id" :firstGridID="paginateInfo.FirstGrid" :lastGridID="paginateInfo.LastGrid" :firstGridIDInList="firstGridIDInList" :lastGridIDInList="lastGridIDInList"  :listOfGrids="listOfGrids" @selected-grid="updateSelectedGrid" @get-grids="getGrids" @previous-page="getPreviousPage" @next-page="getNextPage"/>
  ...
  methods: {
    ...
    updateCurrentGridStatus: function() {
      ...
      for (i = 0; i < this.listOfGrids.length; i++) {
        if (this.id == this.listOfGrids[i].ID && this.currentGridStatus != this.listOfGrids[i].Status) {
    ...
    getPreviousPage: function(gridID) {
      console.log("getPreviousPage")
        axios
          .get('/api/paginate/grid/key/' + gridID + '?offset=-' + this.itemsInList)
          .then(response => {
    ...
    getGrids: function (gridID) {
      ...
      if (gridID == "latest") {
        ...
        urlExtra = ''
      } else if (this.listOfGrids.filter(grid => grid.ID === this.id ).length === 1) {
    ...
    getGridsLoop: function () {
      ...
      for (i = 0; i < this.listOfGrids.length; i++) {
        ...
        if (this.id == this.listOfGrids[i].ID && this.currentGridStatus != status) {
