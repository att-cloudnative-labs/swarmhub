...
func GetInstanceCore(region, instance_type string) (int, error) {
	row := db.QueryRow("select size from portal.region_vm_sizes where name = $1 AND provider_region = (SELECT ID FROM portal.provider_regions WHERE region=$2)", instance_type, region)
	var size int
